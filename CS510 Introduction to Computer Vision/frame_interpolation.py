# CS510 Homework 3: Video Frame Interpolation
# Haomin He
# References: 
...
def find_holes(flow):
    ...
    thisflow = np.asarray(flow)
    # optical flow matrix of shape [h,w,2]
    # get array dimension
    # vector [ux,uy] for each pixel
    thisux = thisflow[:,:,0]
    # Determine it is a hole if a vector length is too long
    for ux in range(thisux.shape[0]):
        for uy in range(thisux.shape[1]):
            if thisux[ux][uy] > 10**9:
                holes[ux][uy] = 0
            else:
                holes[ux][uy] = 1
...
def holefill(flow, holes):
    ...
    while has_hole==1:
        ...
        # set flg to 0 and reset it later
        has_hole = 0
        for y in range(0, h):
            for x in range(0,w):
                # to be completed ...
                ...
                # if there is a hole
                if holes[y][x] == 0:
                    # consider left and top edges of image
                    if x == 0:
                        if y == 0:
                            if(holes[y][x+1] == 1):
                                currentvalue = flow[y][x+1]
                                counter = counter + 1
                            elif (holes[y+1][x] == 1):
                                currentvalue += flow[y+1][x]
                                counter = counter + 1
                            elif(holes[y+1][x+1] == 1):
                                currentvalue += flow[y+1][x+1]
                                counter = counter + 1
                            
                            # check the counter and loop again
                            if counter == 0:
                                has_hole = 1
                            else:
                                # calculate the mean
                                flow[y][x] = currentvalue / counter
                                holes[y][x] = 1
                        # consider bottom edge of image
                        elif y == h - 1:
                            if(holes[y][x+1] == 1):
                                currentvalue = flow[y][x+1]
                                counter = counter + 1
...
def occlusions(flow0, frame0, frame1):
    ...
    for yheight in range(0, height):
        for xwidth in range(0, width):
            thisy = flow0[yheight][xwidth][1]
            thisx = flow0[yheight][xwidth][0]
            checky = int(thisy + yheight + 0.5)
            checkx = int(thisx + xwidth + 0.5)
            if checky >= 0 and checky <= height - 1 and checkx >= 0 and checkx <= width - 1:
                output = np.sum(np.abs(flow0[yheight][xwidth] - flow1[checky][checkx]))
                if output > 0.5:
                    occ1[yheight][xwidth] = 1
            else:
                occ1[yheight][xwidth] = 1
            outy = flow1[yheight][xwidth][0]
            outx = flow1[yheight][xwidth][1]
            if outy > 10**9 or outx > 10**9:
                occ0[yheight][xwidth] = 1
...
def blur(im):
    ...
    blurim = cv2.GaussianBlur(im,(5,5),0)
    return blurim
