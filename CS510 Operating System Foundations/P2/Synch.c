...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Init ()
          -- waiting thread for who wants to get the lock
          waitingThreads = new List [Thread]
          -- the mutex is held by whom
          muheldby = null
        endMethod
      ...
      method Lock ()
          var interState:int
          -- disable interrupts, set interrupt state
          interState = SetInterruptsTo(DISABLED)
          -- if nobody is holding the lock, make the current thread to hold it
          if muheldby == null
            muheldby = currentThread
          else
            -- somebody got the lock already, the current thread waits in the waiting thread, 
            -- and put itself to sleep, no wasting cpu cycles 
            waitingThreads.AddToEnd(currentThread)
            currentThread.Sleep()
          endIf
          -- enable interrupts
          interState = SetInterruptsTo(interState)
        endMethod
      ...
      method Unlock ()
          ...
          -- disable interrupts first, set interrupt state
          interState = SetInterruptsTo(DISABLED)
          
          -- trying to unlock, but we do not own the lock
          if muheldby != currentThread
            FatalError ("current thread is not holding the lock")
          endIf

          -- gets the next thread who wants the lock, sets pointer to it, gives lock to it
          t = waitingThreads.Remove ()
          muheldby = null
          -- if the thread exists, make it ready to run
          if t!=null
            t.status = READY
            readyList.AddToEnd (t)
            muheldby = t
      ...
      method IsHeldByCurrentThread () returns bool
          -- check if current thread is holding the lock or not
          if muheldby == currentThread
            return true -- yes, it holds the lock
          endIf
          return false -- no, it does not hold the lock
        endMethod
