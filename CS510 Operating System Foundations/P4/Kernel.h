header Kernel
  ...
  --  MAX_NUMBER_OF_PROCESSES is 10 in this program
  class ThreadManager
    superclass Object
    fields
      threadTable: array [MAX_NUMBER_OF_PROCESSES] of Thread
      -- when a thread terminates, adding the thread into the free list
      freeList: List [Thread]
      -- turn the ThreadManager into a “monitor”
      threadManagerLock: Mutex
      aThreadBecameFree: Condition   -- condition variable

    methods
      Init ()
      Print ()
      -- When you need a new Thread object, you can invoke threadManager.GetANewThread
